...
const monthlySummaryFunctions = {
  ...
  // async getMonthlySummary(userId, month, year) {
  //   userId = exportedMethods.checkId(userId);
  //   month = parseInt(exportedMethods.checkNumber(month));
  //   year = parseInt(exportedMethods.checkNumber(year));
  
  //   // Get all values directly from update functions
  ...
  //   // Fetch transactions for user in specified month/year
  ...
  //     totalSpent: total
  //   }));
  //   console.log("🧪 Transaction list found:", transactionList);
  //   console.log("🧪 Aggregated breakdown:", breakdownArray);
  ...
  //   if (!updateResult.acknowledged) {
  //     throw new Error("Mongo did not acknowledge the update.");
  //   }
  
  //   console.log("🟦 Updated breakdownByCategory:", breakdownArray);
  ...
  async updateFixedExpenses(userId, month, year) {
    ...
    if (!user || !user.fixedExpenses) throw "User not found or no fixed expenses.";
