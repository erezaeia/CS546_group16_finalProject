...
const incomeFunctions = {
  ...
  async getIncomeByUserIdByMonthAndYear(userId, month, year) {
    ...
    //   throw `Could not fetch all incomes from month and year. pattern: ${pattern}, userid: ${userId}, month: ${month}, year: ${year}`;
    // nastaran
    if (incomeFromUserId.length === 0) {
      console.warn(`No income found for pattern: ${pattern}, userId: ${userId}, month: ${month}, year: ${year}`);
      return [];
    }
    incomeFromUserId = incomeFromUserId.map((element) => {
      element._id = element._id.toString();
